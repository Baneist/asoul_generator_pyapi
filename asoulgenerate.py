...
def fast_sample_sequence(model, context, length, repitition_penalty = 1.0, temperature=1.0, top_k=30, top_p=0.0, device='cpu'):
    ...
    with torch.no_grad():
        for i in trange(length):
            ...
            if next_token.item() == eod_id: break
    return generate
...
device = "cpu"
tokenizer = CpmTokenizer(vocab_file= os.path.dirname(__file__) + "/vocab/chinese_vocab.model")
eod_id = tokenizer.convert_tokens_to_ids('<eod>')
mask_id = tokenizer.mask_token_id
model = GPT2LMHeadModel.from_pretrained(os.path.dirname(__file__) +"/asoul_cpm")
model.to(device)
model.eval()
...
def process(prefix):
    ...
    is_processing = True
    raw_text = prefix
    context_tokens = [mask_id] + tokenizer.convert_tokens_to_ids(tokenizer.tokenize(raw_text))
    generated = 0
